...
sketch_path = Path("/home/ljb/WassersteinSBP/data/ShoeV2/trainD")
...
for img_name in shoe_path.glob("*.png"):
    if f"{img_name.stem}.png" in sketch_names:
        file_names.append(img_name.stem)
shoe_dataset = shoeDataset(shoe_path, file_names, False)
sketch_dataset = shoeDataset(sketch_path, file_names, False)
tgt_imgs_1 = shoe_dataset
tgt_imgs_2 = sketch_dataset
...
train_nums = 1000
train_tgt_imgs_1 = torch.stack([shoe_dataset[i] for i in range(train_nums)], dim=0)
train_tgt_imgs_2  = torch.stack([sketch_dataset[i] for i in range(train_nums)], dim=0)
...
# checkpoint_path = Path('/home/ljb/WassersteinSBP/experiments/gaussian2shoe')
continue_train = True
...
for index, pair in enumerate(train_pair_list):
    ...
    if checkpoint_path is not None and not continue_train:
        ...
        train_dl = DataLoader(combined_dataset, batch_size=batch_size, shuffle=True, pin_memory=True, num_workers=0)
...
# 生成样本
test_num_samples = 25
test_P2_samples = torch.stack([shoe_dataset[-i] for i in range(test_num_samples)], dim=0)
test_P3_samples = torch.stack([sketch_dataset[-i] for i in range(test_num_samples)], dim=0)
